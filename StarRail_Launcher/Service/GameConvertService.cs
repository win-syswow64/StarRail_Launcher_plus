            ...
            if (File.Exists(Path.Combine(App.Current.DataModel.GamePath, "config.ini")))
            {
                ...
                switch (vm.IsMihoyo)
                {
                    case 0:
                        ...
                        if (File.Exists(Path.Combine(GameFolder, $"StarRail_Data/{bilibilisdk}")))
                            File.Delete(Path.Combine(GameFolder, $"StarRail_Data/{bilibilisdk}"));
                    ...
                    // todo bilibili
                     case 1:
                         ...
                         if (!File.Exists(Path.Combine(GameFolder, $"StarRail_Data/{bilibilisdk}")))
                         {
                             try
                             {
                                 string sdkPath = Path.Combine(GameFolder, $"StarRail_Data/{bilibilisdk}");
